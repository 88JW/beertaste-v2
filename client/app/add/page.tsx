...
export default function AddReviewPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('Błąd podczas dodawania recenzji: ' + (error as Error).message);
    } finally {
      setLoading(false);
    }
  };
    ...
    <AuthGuard>
      <main className="min-h-screen bg-gradient-to-br from-amber-50 via-orange-50 to-amber-100">
        <div className="bg-white/80 backdrop-blur-sm px-6 py-4 sticky top-0 z-10">
          <h1 className="text-xl font-bold text-gray-900">Nowa recenzja</h1>
          <p className="text-sm text-gray-500">Oceń swoje piwo</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 pb-24">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-gradient-to-r from-amber-500 to-orange-500 text-white font-bold py-4 rounded-2xl hover:from-amber-600 hover:to-orange-600 transition disabled:opacity-50 shadow-lg"
          >
